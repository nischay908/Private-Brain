...
const NAV: { id: Tab; icon: React.ReactNode; label: string; sub: string }[] = [
  { id: 'chat', label: 'AI Chat', sub: 'Conversational AI',
  ...
  { id: 'notes', label: 'Smart Notes', sub: 'AI-powered notes',
  ...
  { id: 'write', label: 'Writing Studio', sub: 'Draft & polish',
  ...
  { id: 'analyze', label: 'Doc Analyzer', sub: 'PDF · DOCX · TXT',
    icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg> },
  { id: 'code', label: 'Code Helper', sub: 'Explain & debug',
...
const STATUS_COLOR: Record<string, string> = {
  ...
  downloading: '#F59E0B', loading: '#F59E0B', initializing: '#F59E0B', idle: '#6B7280',
};
      ...
      <aside className="pb-sidebar">
        ...
        <div className="pb-logo">
          <div className="pb-logo-mark">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <circle cx="12" cy="12" r="10" stroke="url(#lg2)" strokeWidth="2"/>
              <path d="M8 12h8M12 8v8" stroke="url(#lg2)" strokeWidth="2" strokeLinecap="round"/>
              <defs>
                <linearGradient id="lg2" x1="0" y1="0" x2="24" y2="24">
                  <stop offset="0%" stopColor="#818CF8"/><stop offset="100%" stopColor="#22D3EE"/>
                </linearGradient>
              </defs>
            </svg>
          </div>
          <span className="pb-logo-text">Private<strong>Brain</strong></span>
        </div>
        ...
        <div className="pb-model-pill">
          ...
          <span className="pb-model-label">
            {model.status === 'ready' ? 'Llama 3.2 · On-Device' : model.progressLabel || 'Loading model…'}
          </span>
        </div>

        {/* Nav */}
        <nav className="pb-nav">
          <span className="pb-nav-section">Workspace</span>
          {NAV.map(item => (
            <button key={item.id} className={`pb-nav-item ${activeTab === item.id ? 'active' : ''}`} onClick={() => onTabChange(item.id)}>
